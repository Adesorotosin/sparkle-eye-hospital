...
export default function AccessControlPage() {
  ...
  const handleGroupToggle = (groupKeys: string[], enable: boolean) => {
    setDraftPermissions((prev) => {
      const updated = { ...prev };
      groupKeys.forEach((k) => {
        updated[k] = enable;
  ...
  const handleSavePermissions = () => {
    ...
    setHasChanges(false);
    showToast(`Access permissions updated for ${activeStaff.name}`);
  };
  ...
  const filteredStaff = useMemo(() => {
    return staffList.filter((member) => {
      ...
      if (activeTab === "doctors") {
        return /doctor|ophthalmologist|specialist|anesthetist/i.test(member.role);
      }
      if (activeTab === "nurses") return /nurse/i.test(member.role);
      if (activeTab === "pharmacists") return /pharmacist/i.test(member.role);
      if (activeTab === "frontdesk") return /front desk/i.test(member.role);
      if (activeTab === "admins") return /admin/i.test(member.role);
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 flex flex-col">
      ...
      {notification && (
        <div className="fixed top-4 right-4 z-50 bg-slate-900 text-white px-4 py-3 rounded-xl shadow-lg flex items-center gap-3 animate-in fade-in slide-in-from-top-2 duration-200">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-slate-100 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all placeholder:text-slate-400"
            />
          </div>

          <a
            href="/admin/staff/onboarding"
            className="hidden sm:inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold px-4 py-2.5 rounded-lg transition-colors whitespace-nowrap shadow-sm"
          >
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-white border-r border-slate-200">
            ...
            {TAB_FILTERS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`py-3.5 px-4 text-sm font-semibold whitespace-nowrap border-b-2 transition-colors flex items-center gap-1.5 cursor-pointer ${
                    isActive
          ...
          <div className="flex-1 overflow-y-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/80 border-b border-slate-200 text-[11px] font-bold tracking-wider text-slate-500 uppercase sticky top-0 bg-slate-50 z-1">
          ...
          <div className="w-full lg:w-[420px] bg-white border-l border-slate-200 flex flex-col shrink-0 shadow-lg lg:shadow-none">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6">
              {PERMISSION_GROUPS.map((group) => {
                ...
                const allGroupEnabled = groupKeys.every((k) => draftPermissions[k]);
                ...
                return (
                  <div key={group.id} className="space-y-3">
                    <div className="flex items-center justify-between border-b border-slate-100 pb-1.5">
                      ...
                      <button
                        type="button"
                        onClick={() => handleGroupToggle(groupKeys, !allGroupEnabled)}
            ...
            <div className="p-4 border-t border-slate-200 bg-slate-50 flex flex-col gap-3">
              <div className="flex items-center justify-between gap-3">
                ...
                <button
                  ...
                  disabled={!hasChanges}
                  className={`text-xs font-semibold px-4 py-2.5 rounded-lg shadow-sm transition-all flex items-center gap-1.5 ${
                    hasChanges
              ...
              <p className="text-[11px] text-slate-400 text-center font-medium">
                Last modified: Sep 3, 2026 by Admin Sarah Ogundimu
              </p>
            </div>
          </div>
        )}
