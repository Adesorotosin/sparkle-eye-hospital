...
function PharmacyContent() {
  ...
  const handleDrugNameChange = (value: string) => {
    setNewDrugStock({ ...newDrugStock, name: value });
    ...
    drugSearchTimer.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch(`/api/drug-search?q=${encodeURIComponent(value)}`);
            ...
            <form onSubmit={handleAddDrugStock} className="space-y-4 text-xs">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-slate-800 focus:outline-none focus:border-purple-600"
                />
                {showDrugSuggestions && (isSearchingDrugs || drugSuggestions.length > 0) && (
                    ...
                    {isSearchingDrugs ? (
                      ...
                      drugSuggestions.map((suggestion) => (
                        <button
                          ...
                          key={suggestion}
                          onClick={() => {
                            setNewDrugStock({ ...newDrugStock, name: suggestion });
                            setShowDrugSuggestions(false);
                          }}
