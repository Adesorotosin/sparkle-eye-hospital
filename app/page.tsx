...
export default function LoginPage() {
  ...
  const [rememberWorkstation, setRememberWorkstation] =
    useState(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      /*
       * The server determines the correct dashboard
       * based on the authenticated staff member's role.
       */
      ...
      <div className="w-full md:w-[40%] lg:w-[35%] bg-[#222222] text-white p-8 lg:p-12 flex flex-col justify-between min-h-[300px] md:min-h-screen">
        ...
        <div>
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-zinc-800/80 border border-zinc-700/60 text-xs font-medium text-zinc-300">
            ...
            <span>
              System Operational | Local Server Online
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="w-full max-w-md bg-white rounded-2xl p-8 lg:p-10 shadow-xl shadow-slate-200/50 border border-slate-100">
          ...
          <div className="mb-8">
            ...
            <p className="text-xs text-slate-500">
              Please enter your security credentials to access
              your portal.
            </p>
          </div>
          ...
          >
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-3 px-4 bg-[#6D4AFF] hover:bg-[#5B3CE1] active:bg-[#4C2ECC] disabled:bg-slate-300 text-white text-sm font-semibold rounded-xl shadow-md shadow-[#6D4AFF]/25 transition duration-150 ease-in-out mt-2 flex items-center justify-center gap-2 cursor-pointer disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <>
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 flex items-center justify-center gap-2 text-xs text-slate-400">
            ...
            <span>
              Role-Based Access Enforced • Audit Trail Active
            </span>
          </div>
        </div>
      </div>
